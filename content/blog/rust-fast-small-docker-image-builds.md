...
In this post I'm going to demonstrate how to create small, quickly built [Docker Images](https://hub.docker.com/explore/) for [Rust](https://www.rust-lang.org/) applications.

We'll start by creating a simple test application, and then building and iterating on a [Dockerfile](https://docs.docker.com/engine/reference/builder/).

## Requirements

Ensure you have the following installed:

- [rustup](https://rustup.rs) v1.14.0+
- [docker](https://docker.com) v17.06.2+
...
The following is a starting place we'll use for our [docker build](https://docs.docker.com/engine/reference/commandline/build/), create a file named `Dockerfile` in the current directory:
...
At the time of writing this blog post, Rust's package manager [cargo](https://github.com/rust-lang/cargo) has an issue where [it does not have a --depedencies-only option to build depedencies independently](https://github.com/rust-lang/cargo/issues/2644).
...
As an additional problem, while the [rust:latest](https://hub.docker.com/r/library/rust/tags/) Docker image is great for building, it's a fairly large image coming in at over 1.5GB in size.
...
We've added a new [crate](https://crates.io) as a dependency to our project named [rand](https://crates.io/crates/rand) which provides convenient random number generation utilities.
...
The [rust:latest](https://hub.docker.com/r/library/rust/tags/) image has all the tools we need to build our project, but is over 1.5GB in size. We can improve the image size by using [Alpine Linux](https://alpinelinux.org/) which is an excellent small Linux distribution.

The Alpine team provides [a docker image](https://hub.docker.com/_/alpine/) which is only several megabytes in size and still has some shell functionality for debugging and can be used as a small base image for our Rust builds.

Using [multi-stage docker build](https://docs.docker.com/develop/develop-images/multistage-build/)s we can use [rust:latest](https://hub.docker.com/r/library/rust/tags/) to do our build work, but then simply copy the app into a final build stage based on [alpine:latest](https://hub.docker.com/_/alpine/):
...
If you're familiar with [ldd](https://en.wikipedia.org/wiki/Ldd_(Unix)) you can run the following to see that we're missing shared libraries for our application:

```
docker run --rm -it myapp ldd /usr/local/bin/myapp
```

In the above examples we show how to avoid rebuilding depdencies on every `src/` file change, and how to reduce our image footprint from 1.5GB+ to several megabytes, however our build doesn't currently work because we need to build against [MUSL Libc](https://www.musl-libc.org/) which is a lightweight, fast standard library available as the default in `alpine:latest`.

Beyond that, we also want to make sure that our application runs as an unprivileged user inside the container so as to adhere to the [principle of least privilege](https://en.wikipedia.org/wiki/Principle_of_least_privilege).

### Building for MUSL Libc

To build for MUSL libc we'll need to install the `x86_64-unknown-linux-musl` [target](https://doc.rust-lang.org/rustc/targets/built-in.html) so that `cargo` can be flagged to build for it with `--target`. We'll also need to flag Rust to use the `musl-gcc` linker.
...
To follow [principle of least privilege](https://en.wikipedia.org/wiki/Principle_of_least_privilege), let's create a user named "myapp" which we'll use to run `myapp` as instead of as the `root` user.
...
From here see my demo on [deploying Rust to Kubernetes on DC/OS with Skaffold](https://github.com/shaneutt/dcos-k8s-rust-skaffold-demo). Utilizing some of the techniques in that demo, you could automate deployment of your application to [Kubernetes](https://kubernetes.io) for testing on a local [minikube](https://kubernetes.io/docs/setup/minikube/) system using [Skaffold](https://github.com/GoogleContainerTools/skaffold).
